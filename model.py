...
class GCNEncoder(nn.Module):
    def __init__(self, input_dim, generate_dim, edge_dim, dropout=0.5, beta=1) -> None:
        ...
        self.gcn = GraphAttention(
            self.input_dim, self.generate_dim, self.edge_dim, dropout
        )
...
class Fusion(nn.Module):
    def __init__(self, fusion_dim=1024, category=3) -> None:
        super(Fusion, self).__init__()
        self.dropout = nn.Dropout(0.1)
        self.classification = nn.Sequential(
            nn.Linear(fusion_dim, category),
            nn.Softmax(),
        )

    def forward(self, x, y):
        classify = self.dropout((x + y) / 2)
        classify = self.classification(classify)
        return classify


class Fusion2(nn.Module):
    def __init__(self, fusion_dim=1024) -> None:
        super(Fusion2, self).__init__()
        self.dropout = nn.Dropout(0.1)
        self.classification = nn.Sequential(
            nn.Linear(fusion_dim, 90),
            nn.Tanh(),
        )
...
class TransformerEncoder(nn.Module):
    def __init__(
        self, Q_dim, K_dim, V_dim, d_model=1024, dim_feedforward=2048, dropout=0.5
    ) -> None:
